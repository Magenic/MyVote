...
module MyVote.Controllers {
    ...
    export interface PollResultScope extends ng.IScope {
        pollResult: MyVote.Services.AppServer.Models.PollResult;
        infoMessage: string;
        errorMessage: string;
        ...
        canDelete(): boolean;
        delete(): void;
        submitComment(parentComment: MyVote.Services.AppServer.Models.PollResultComment, text: string): void;
    }
    ...
    export class PollResultCtrl {
        ...
        constructor($scope: PollResultScope, $rootScope: ng.IRootScopeService, $routeParams: PollResultParams, $q: ng.IQService, $log: ng.ILogService,
            ...
            myVoteService.getPollResult($routeParams.pollId).then(
                (result: MyVote.Services.AppServer.Models.PollResult) => {
                    $scope.infoMessage = null;
                    $scope.pollResult = result;

                    // set up chart data - object matrix {}[][]
                    var categories: string[] = [];
                    var resultValues: number[] = $scope.pollResult.Results.map((resultItem: MyVote.Services.AppServer.Models.PollResultItem, index: number) => {
            ...
            $scope.submitComment = (parentComment: MyVote.Services.AppServer.Models.PollResultComment, text: string)=> {
                if (!text) {
                    return;
                }
                var newComment: MyVote.Services.AppServer.Models.PollResultComment = {
