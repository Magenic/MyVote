	...
	public sealed class PollControllerTests
	{
		...
		public void GetPoll()
		{
			...
			var controller = new PollController(new Lazy<IObjectFactory<IPoll>>(), 
				...
				new Lazy<IObjectFactory<IPollSearchResults>>(() => pollSearchResultsFactory.Object),
				Mock.Of<IMyVoteAuthentication>());
		...
		public void GetSinglePoll()
		{
			...
			var controller = new PollController(new Lazy<IObjectFactory<IPoll>>(() => pollFactory.Object),
				new Lazy<IObjectFactory<IPollOption>>(),
		...
		private static async Task TestPollPut(Action<Mock<IObjectFactory<IPoll>>, Poll, Mock<IPoll>> pollFactorySetup,
			Func<PollController, Poll, Task> pollControllerInvocation)
		{
			...
			poll.SetupGet(_ => _.IsSavable).Returns(true);
			poll.Setup(_ => _.SaveAsync()).Returns(Task.FromResult<object>(poll.Object));
			...
			var controller = new PollController(new Lazy<IObjectFactory<IPoll>>(() => pollFactory.Object),
				new Lazy<IObjectFactory<IPollOption>>(),
				new Lazy<IObjectFactory<IPollSearchResults>>(), auth.Object);
