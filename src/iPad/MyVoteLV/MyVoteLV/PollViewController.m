...
static NSInteger const MALSectionsForAnsweredPoll = 4;
...
static NSInteger const MALPostCommentSection = 1;
static NSInteger const MALSubmitCommentSection = 2;
static NSInteger const MALCommentsSection = 3;

// row heights
static CGFloat const MALUnansweredRowHeight = 60.0f;
static CGFloat const MALStatsRowHeight = 300.0f;
static CGFloat const MALPostCommentRowHeight = 60.0f;
...
@interface PollViewController () <UIPopoverControllerDelegate, IGChartViewDelegate, UITextFieldDelegate, UIAlertViewDelegate, ReplyCommentDelegate>

@property BOOL hasViewBeenSetup;
@property BOOL hasUserAnsweredPoll;
@property (strong, nonatomic) UIPopoverController *popover;
@property (strong, nonatomic) UIPopoverController *replyCommentPopover;
...
- (void)viewDidLoad
{
    ...
    } else if (self.pollSummary.imageLink.integerValue > 0) {
        NSInteger blobID = self.pollSummary.imageLink.integerValue;
        [[MyVoteService sharedInstance] getImageWithID:[NSNumber numberWithInteger:blobID] completion:^(NSString *imageString) {
            if (imageString != (id)[NSNull null]) {
                NSData *data = [NSData dataFromBase64String:imageString];
                UIImage *image = [UIImage imageWithData:data];
                self.pollHeaderView.pollImageView.image = image;
            }
        }];
    } else {
    ...
    // fetch this specific poll
    //[self fetchThisPoll];
    
    // add subview over tableview
    //UIView *view = [[UIView alloc] initWithFrame:CGRectMake(500, 500, 500, 500)];
...
- (void)replyToComment:(UIButton *)sender
{
    ...
    if (indexPath) {
        ...
        CommentReplyViewController *viewControllerForPopover = [self.storyboard instantiateViewControllerWithIdentifier:@"Reply Comment Popover"];
        viewControllerForPopover.delegate = self;
        viewControllerForPopover.comment = postedComment;
        
        self.replyCommentPopover = [[UIPopoverController alloc] initWithContentViewController:viewControllerForPopover];
        [self.replyCommentPopover presentPopoverFromRect:cell.frame
                                                  inView:self.view
...
{
    // dismiss popover
    [self.replyCommentPopover dismissPopoverAnimated:YES];
    ...
    if (self.hasViewBeenSetup) {
        if (self.hasUserAnsweredPoll) {
            ...
            if (section == MALStatSection) {
                return 1;
            } else if (section == MALPostCommentSection) {
                return 1;
            } else if (section == MALSubmitCommentSection) {
                return 1;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.hasUserAnsweredPoll) {
        ...
        } else if (indexPath.section == MALPostCommentSection || indexPath.section == MALSubmitCommentSection) {
            // post comment and submit comment are equal
            return MALPostCommentRowHeight;
        } else if (section == MALCommentsSection) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.hasUserAnsweredPoll) {
        if (indexPath.section == MALSubmitCommentSection) {
            // submit new comment
            [self postComment:[self createNewCommentForThisPoll]];
        }
    } else {
...
- (UITableViewCell *)configureAnsweredCellsWithTableView:(UITableView *)tableView forIndexPath:(NSIndexPath *)indexPath
{
    ...
    static NSString *PostCommentCellIdentifier = @"Post Comment Cell";
    ...
    if (indexPath.section == MALStatSection) {
        ...
        return cell;
    } else if (indexPath.section == MALPostCommentSection) {
        UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:PostCommentCellIdentifier forIndexPath:indexPath];
        self.commentTextField = (UITextField *)[cell viewWithTag:MALTextfieldTag];
        
        return cell;
    } else if (indexPath.section == MALSubmitCommentSection) {
        UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:SubmitCellIdentifier forIndexPath:indexPath];
        cell.textLabel.text = NSLocalizedString(@"Submit My Comment", nil);
