...
module MyVote.Controllers {
    ...
    export interface ViewPollScope extends ng.IScope {
        pollId: number;
        currentPoll: MyVote.AppServer.Models.Poll;
        infoMessage: string;
        errorMessage: string;
    ...
    export class ViewPollCtrl {
        ...
        constructor(
            ...
            myVoteService.getPollResponse($scope.pollId, authService.userId)
                ...
                }).then((poll: MyVote.AppServer.Models.Poll) => {
            ...
            $scope.canSubmit = () => {
                ...
                var selectedCount = 0;
                angular.forEach($scope.currentPoll.PollOptions, (po: MyVote.AppServer.Models.PollOption) => {
                    if (po.Selected)
                        selectedCount++;
                });
            ...
            $scope.submit = () => {
                var pollResponse: MyVote.AppServer.Models.PollResponse = {
                    PollID: $scope.currentPoll.PollID,
                    UserID: authService.userId,
                    Comment: null,
                    ResponseItems: $scope.currentPoll.PollOptions.map(po => {
                        var responseItem: MyVote.AppServer.Models.ResponseItem = {
