...
namespace MyVote.BusinessObjects.Tests
{
	public sealed class PollLifecycleTests
	{
		...
		public void Create()
		{
			...
			pollOptions.Handle(_ => _.CreateChild()).Returns(new BusinessList<IPollOption>());
			...
			builder.Register<IObjectFactory<IPollOption>>(_ => Rock.Make<IObjectFactory<IPollOption>>(
				new RockOptions(allowWarnings: AllowWarnings.Yes)));
		...
		public void Fetch()
		{
			...
			var pollOptionsFactory = Rock.Create<IObjectFactory<BusinessList<IPollOption>>>(
				new RockOptions(allowWarnings: AllowWarnings.Yes));
			pollOptionsFactory.Handle(_ => _.FetchChild()).Returns(new BusinessList<IPollOption>());

			var pollOption = Rock.Create<IPollOption>(
				new RockOptions(allowWarnings: AllowWarnings.Yes));
			pollOption.Handle(nameof(IPollOption.IsChild), () => true);
			pollOption.Handle(_ => _.SetParent(Arg.IsAny<Poll>()));
			pollOption.Handle<int>(nameof(IPollOption.EditLevelAdded), _ => { }, 2);
			pollOption.Handle(nameof(IPollOption.EditLevel), () => 0, 2);
			pollOption.Handle(nameof(IPollOption.IsBusy), () => false, 2);
			var pollOptionChunk = pollOption.Make();

			var pollOptionFactory = Rock.Create<IObjectFactory<IPollOption>>(
				new RockOptions(allowWarnings: AllowWarnings.Yes));
			pollOptionFactory.Handle(_ => _.FetchChild(Arg.IsAny<object[]>())).Returns(pollOptionChunk);

			var entities = Rock.Create<IEntitiesContext>(
				new RockOptions(allowWarnings: AllowWarnings.Yes));
			entities.Handle(nameof(IEntitiesContext.Mvpoll), () => new InMemoryDbSet<Mvpoll>() { entity });
			...
			builder.Register<IObjectFactory<IPollOption>>(_ => pollOptionFactory.Make());
				...
				.Bind(() => ApplicationContext.DataPortalActivator))
			{
				...
				poll.PollOptions[0].Should().BeSameAs(pollOptionChunk);
			}

			entities.Verify();
			pollOptionsFactory.Verify();
			pollOptionFactory.Verify();
		}
		...
		public void Insert()
		{
			...
			var entities = Rock.Create<IEntitiesContext>(
				new RockOptions(allowWarnings: AllowWarnings.Yes));
			entities.Handle(nameof(IEntitiesContext.MvreportedPoll), 2);
			...
			entities.Handle(_ => _.SaveChanges(),
				() =>
				{
					polls.LocalData[0].PollId = pollId;
					return 1;
				}, 3);
			entities.Handle(_ => _.Dispose());
			...
			var pollOptionsFactory = Rock.Create<IObjectFactory<BusinessList<IPollOption>>>(
				new RockOptions(allowWarnings: AllowWarnings.Yes));
			pollOptionsFactory.Handle(_ => _.CreateChild()).Returns(new BusinessList<IPollOption>());
		...
		public void Update()
		{
			...
			var entities = Rock.Create<IEntitiesContext>(
				new RockOptions(allowWarnings: AllowWarnings.Yes));
			entities.Handle(nameof(IEntitiesContext.Mvpoll), () => new InMemoryDbSet<Mvpoll> { entity });
			entities.Handle(_ => _.SetState(Arg.IsAny<Mvpoll>(), EntityState.Modified));
		...
		public void Delete()
		{
			...
			using (principal.Make().Bind(() => ApplicationContext.User))
			{
					...
					.Bind(() => ApplicationContext.DataPortalActivator))
				{
					...
					polls.LocalData.Count.Should().Be(2);
					var deletedPoll = polls.LocalData[1];
